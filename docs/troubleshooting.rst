...
**Cause:** The API token lacks the required permissions for the endpoint being

accessed.


**Fix:** Verify the token has the necessary read permissions. See the

:doc:`Permissions Guide <permissions-guide>` for the minimum and recommended
...
**Cause:** The ``--zones`` filter excluded all zones, or the token cannot

enumerate zones in the account.
...
**Cause:** The ``--checks`` flag included category names that FlareInspect does

not recognize.


**Fix:** Use only supported categories. Run ``flareinspect help assess`` for the

full list of 21 categories.
...
**Cause:** The ``--compliance`` flag or API request used a framework name outside
...
**Cause:** The token may be scoped to a single Cloudflare account or

organization. If your token can only enumerate one zone, the assessment will

only cover that zone.
...
**Cause:** An API error occurred during assessment. Check the error message for
