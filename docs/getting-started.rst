...
8. Copy the token value — you will pass it to FlareInspect via the ``--token`` flag or the ``CLOUDFLARE_TOKEN`` environment variable.
   ...
   **Tip:** Store the token securely. Avoid committing it to source repositories. Use environment variables or a secret manager in CI/CD pipelines.
...
.. rubric:: Scope to Specific Zones
...
.. rubric:: Run Specific Check Categories
